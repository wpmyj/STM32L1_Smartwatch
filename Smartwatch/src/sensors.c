...
void sensors_t(void){
    ...
    for(;;){
        // Read temperature
        taskENTER_CRITICAL();
        sensors_startSHT21(CMD_TEMPERATURE);
        taskEXIT_CRITICAL();
        vTaskDelay(100);
        taskENTER_CRITICAL();
        sensor.temperature = sensors_getSHT21Value() / 1000;
        taskEXIT_CRITICAL();
        // Read humidity
        taskENTER_CRITICAL();
        sensors_startSHT21(CMD_HUMIDITY);
        taskEXIT_CRITICAL();
        vTaskDelay(100);
        taskENTER_CRITICAL();
        sensor.humidity = sensors_getSHT21Value() / 1000;
        taskEXIT_CRITICAL();
        // Send structure to display task
        xQueueSend(sensor_displayQ, &sensor, 0);
        vTaskDelay(10000);
    }
